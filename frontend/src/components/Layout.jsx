...
const Layout = ({onLogout,user}) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const {data} = await axios.get("http://localhost:5173/api/tasks/gp",{
  ...
  const StatCard = ({ title, value, icon}) => (
    <div className=' p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-shadow duration-300 hover:border-purple-100 group'>
                ...
                <div className='relative pt-1'>
                  <div className='flex gap-1.5 items-center'>
                    <div className=' flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
                      <div className=' h-full  bg-gradient-to-r from-fuchsia-500 to-purple-600 transition-all duration-300' style={{ width: `${stats.completionPercentage}%` }}/>
                    </div>
                  </div>
                </div>
              </div>
